...
# Figure S5: heatmap with NP vs. PAX module correlations by age category
...
metadata_np_Pos$alias_study_id <- paste(metadata_np_Pos$alias_study_id, "M0", sep=".")
metadata_np_Pos_0to5 <- subset(metadata_np_Pos, age_cat=="0-5 years")
ids_0to5 <- metadata_np_Pos_0to5$alias_study_id
metadata_np_Pos_6to13 <- subset(metadata_np_Pos, age_cat=="6-13 years")
ids_6to13 <- metadata_np_Pos_6to13$alias_study_id
metadata_np_Pos_14to20 <- subset(metadata_np_Pos, age_cat=="14-20 years")
ids_14to20 <- metadata_np_Pos_14to20$alias_study_id

ssgsea_0to5 <- ssgsea_all[row.names(ssgsea_all) %in% ids_0to5,]
corr_mat_0to5 <- rcorr(as.matrix(ssgsea_0to5),type="pearson")
diag(corr_mat_0to5$P) <- 0
r_mat_0to5 <- corr_mat_0to5$r
r_mat_0to5 <- r_mat_0to5[1:30,31:60]
row.names(r_mat_0to5) <- gsub(".NSB","",row.names(r_mat_0to5))
colnames(r_mat_0to5) <- gsub(".PAX","",row.names(r_mat_0to5))
p_mat_0to5 <- corr_mat_0to5$P
p_mat_0to5 <- p_mat_0to5[1:30,31:60]
row.names(p_mat_0to5) <- gsub(".NSB","",row.names(p_mat_0to5))
colnames(p_mat_0to5) <- gsub(".PAX","",row.names(p_mat_0to5))
corr_tab_0to5 <- flattenCorrMatrix(corr_mat_0to5$r, corr_mat_0to5$P)
corr_tab_0to5 <- dplyr::filter(corr_tab_0to5, grepl('.NSB', row))
corr_tab_0to5 <- dplyr::filter(corr_tab_0to5, grepl('.PAX', column))
write.csv(corr_tab_0to5, "5_NP_vs_PAX_Correlations/NP_vs_PAX_correlations_0to5.csv")

png(file="5_NP_vs_PAX_Correlations/Correlations_0to5.png", width = 7.5, height = 5, units = 'in', res = 1200)
corrplot(r_mat_0to5, type="full", order="original", p.mat = p_mat_0to5, sig.level = 0.05, insig = "blank", tl.cex = 0.5, tl.srt = 45, tl.col="black", 
         col = COL2('PuOr', 10))
mtext("Peripheral blood",side=3,line=3,cex=0.75,adj=0.58)
mtext("Upper respiratory",side=2,line=1,cex=0.75,adj=0.40)
dev.off()

ssgsea_6to13 <- ssgsea_all[row.names(ssgsea_all) %in% ids_6to13,]
corr_mat_6to13 <- rcorr(as.matrix(ssgsea_6to13),type="pearson")
diag(corr_mat_6to13$P) <- 0
r_mat_6to13 <- corr_mat_6to13$r
r_mat_6to13 <- r_mat_6to13[1:30,31:60]
row.names(r_mat_6to13) <- gsub(".NSB","",row.names(r_mat_6to13))
colnames(r_mat_6to13) <- gsub(".PAX","",row.names(r_mat_6to13))
p_mat_6to13 <- corr_mat_6to13$P
p_mat_6to13 <- p_mat_6to13[1:30,31:60]
row.names(p_mat_6to13) <- gsub(".NSB","",row.names(p_mat_6to13))
colnames(p_mat_6to13) <- gsub(".PAX","",row.names(p_mat_6to13))
corr_tab_6to13 <- flattenCorrMatrix(corr_mat_6to13$r, corr_mat_6to13$P)
corr_tab_6to13 <- dplyr::filter(corr_tab_6to13, grepl('.NSB', row))
corr_tab_6to13 <- dplyr::filter(corr_tab_6to13, grepl('.PAX', column))
write.csv(corr_tab_6to13, "5_NP_vs_PAX_Correlations/NP_vs_PAX_correlations_6to13.csv")

png(file="5_NP_vs_PAX_Correlations/Correlations_6to13.png", width = 7.5, height = 5, units = 'in', res = 1200)
corrplot(r_mat_6to13, type="full", order="original", p.mat = p_mat_6to13, sig.level = 0.05, insig = "blank", tl.cex = 0.5, tl.srt = 45, tl.col="black", 
         col = COL2('PuOr', 10))
mtext("Peripheral blood",side=3,line=3,cex=0.75,adj=0.58)
mtext("Upper respiratory",side=2,line=1,cex=0.75,adj=0.40)
dev.off()

ssgsea_14to20 <- ssgsea_all[row.names(ssgsea_all) %in% ids_14to20,]
corr_mat_14to20 <- rcorr(as.matrix(ssgsea_14to20),type="pearson")
diag(corr_mat_14to20$P) <- 0
r_mat_14to20 <- corr_mat_14to20$r
r_mat_14to20 <- r_mat_14to20[1:30,31:60]
row.names(r_mat_14to20) <- gsub(".NSB","",row.names(r_mat_14to20))
colnames(r_mat_14to20) <- gsub(".PAX","",row.names(r_mat_14to20))
p_mat_14to20 <- corr_mat_14to20$P
p_mat_14to20 <- p_mat_14to20[1:30,31:60]
row.names(p_mat_14to20) <- gsub(".NSB","",row.names(p_mat_14to20))
colnames(p_mat_14to20) <- gsub(".PAX","",row.names(p_mat_14to20))
corr_tab_14to20 <- flattenCorrMatrix(corr_mat_14to20$r, corr_mat_14to20$P)
corr_tab_14to20 <- dplyr::filter(corr_tab_14to20, grepl('.NSB', row))
corr_tab_14to20 <- dplyr::filter(corr_tab_14to20, grepl('.PAX', column))
write.csv(corr_tab_14to20, "5_NP_vs_PAX_Correlations/NP_vs_PAX_correlations_14to20.csv")

png(file="5_NP_vs_PAX_Correlations/Correlations_14to20.png", width = 7.5, height = 5, units = 'in', res = 1200)
corrplot(r_mat_14to20, type="full", order="original", p.mat = p_mat_14to20, sig.level = 0.05, insig = "blank", tl.cex = 0.5, tl.srt = 45, tl.col="black", 
         col = COL2('PuOr', 10))
mtext("Peripheral blood",side=3,line=3,cex=0.75,adj=0.58)
mtext("Upper respiratory",side=2,line=1,cex=0.75,adj=0.40)
dev.off()
...
library(magick)
correlations_0to5 <- image_read("5_NP_vs_PAX_Correlations/Correlations_0to5.png") %>% image_ggplot()
correlations_6to13 <- image_read("5_NP_vs_PAX_Correlations/Correlations_6to13.png") %>% image_ggplot()
correlations_14to20 <- image_read("5_NP_vs_PAX_Correlations/Correlations_14to20.png") %>% image_ggplot()
title_0to5 <- ggdraw() + draw_label("0-5 years", size=12, fontface='bold')
title_6to13 <- ggdraw() + draw_label("6-13 years", size=12, fontface='bold')
title_14to20 <- ggdraw() + draw_label("14-20 years", size=12, fontface='bold')
plot_0to5 <- plot_grid(title_0to5, correlations_0to5, ncol=1, align="v", rel_heights = c(0.1,1))
plot_6to13 <- plot_grid(title_6to13, correlations_6to13, ncol=1, align="v", rel_heights = c(0.1,1))
plot_14to20 <- plot_grid(title_14to20, correlations_14to20, ncol=1, align="v", rel_heights = c(0.1,1))
plot_row1 <- plot_grid(plot_0to5, NULL, plot_6to13, nrow=1, align="h", rel_widths=c(1,-0.13,1))
plot_row2 <- plot_grid(plot_14to20, NULL, NULL, nrow=1, align="h", rel_widths=c(1,-0.13,1))

png(file="Figures/Figure_S5.png", width = 13, height = 10, units = 'in', res = 1200)
plot_grid(plot_row1, plot_row2, labels=NULL, nrow=2, ncol=1, align="v") 
dev.off()