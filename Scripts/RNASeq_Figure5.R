...
colnames(modules)[which(names(modules) == "BCR Signaling")] <- "B cell receptor signaling"
colnames(modules)[which(names(modules) == "NF-kappaB Signaling")] <- "NF-kappaB signaling"
colnames(modules)[which(names(modules) == "TCR Signaling")] <- "T cell receptor signaling"
...
pathway_order <- c("Innate immune cell activation","Interferon response","Type I interferon signaling","Type II interferon signaling","Type III interferon signaling",
                   ...
                   "T cell receptor signaling","B cell receptor signaling","NK cell activity","Mononuclear cell migration","Lymphocyte trafficking",
                   "Immune memory")
...
png(file="Statistical_Analyses/Figures/Figure_5.png", width = 7.5, height = 5, units = 'in', res = 1200)
corrplot(r_mat, type="full", order="original", p.mat = p_mat, sig.level = 0.05, insig = "blank", tl.cex = 0.62, tl.srt = 45, tl.col="black", 
         col = COL2('PuOr', 10))
mtext("Peripheral blood",side=3,line=3,cex=0.8,adj=0.58)
mtext("Upper respiratory",side=2,line=1,cex=0.8,adj=0.38)
dev.off()