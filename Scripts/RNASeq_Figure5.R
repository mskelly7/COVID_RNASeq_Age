...
write.csv(corr_tab, "Statistical_Analyses/5_NP_vs_PAX_Correlations/NP_vs_PAX_correlations_all.csv")

# Generate Figure 5 focusing on select modules

ssgsea_np <- gsva(data_np_Pos_log2, modules, method="ssgsea")
ssgsea_np <- as.data.frame(t(ssgsea_np))
pathway_order <- c("Innate immune cell activation","Interferon response","Type I interferon signaling","Type II interferon signaling","Type III interferon signaling",
                   "RNA sensing","Phagocytosis","Myeloid activation",
                   "Complement system","Coagulation","Leukotriene/prostaglandin inflammation","Adaptive immune response",
                   "TCR signaling","BCR signaling","NK cell activity","Mononuclear cell migration","Lymphocyte trafficking",
                   "Immune memory")
...
ssgsea_all$Row.names <- NULL
corr_mat <- rcorr(as.matrix(ssgsea_all),type="pearson")
corr_mat <- rcorr_padjust(corr_mat, method="BH")
diag(corr_mat$P) <- 0
r_mat <- corr_mat$r
r_mat <- r_mat[1:18,19:36]
row.names(r_mat) <- gsub(".NSB","",row.names(r_mat))
colnames(r_mat) <- gsub(".PAX","",row.names(r_mat))
p_mat <- corr_mat$P
p_mat <- p_mat[1:18,19:36]
row.names(p_mat) <- gsub(".NSB","",row.names(p_mat))
colnames(p_mat) <- gsub(".PAX","",row.names(p_mat))
corr_tab <- flattenCorrMatrix(corr_mat$r, corr_mat$P)
corr_tab <- dplyr::filter(corr_tab, grepl('.NSB', row))
...
#corrplot(r_mat, type="full", order="original", p.mat = p_mat, sig.level = 0.05, insig = "blank", tl.cex = 0.62, tl.srt = 45, tl.col="black", 
#         col = COL2('PuOr', 10))
#mtext("Peripheral blood",side=3,line=3,cex=0.8,adj=0.58,font=2)
#mtext("Upper respiratory",side=2,line=1,cex=0.8,adj=0.38,font=2)
#dev.off()

# ADDITION OF TITLE

png(file="Statistical_Analyses/5_NP_vs_PAX_Correlations/Correlation_plot.png", width = 7.5, height = 5, units = 'in', res = 1200)
corrplot(r_mat, type="full", order="original", p.mat = p_mat, sig.level = 0.05, insig = "blank", tl.cex = 0.62, tl.srt = 45, tl.col="black", 
         col = COL2('PuOr', 10))
mtext("Peripheral blood",side=3,line=3,cex=0.7,adj=0.58,font=2)
mtext("Upper respiratory",side=2,line=1,cex=0.7,adj=0.38,font=2)
dev.off()

library(magick)
fig5_plot <- image_read("Statistical_Analyses/5_NP_vs_PAX_Correlations/correlation_plot.png") %>% image_ggplot()
fig5_title1 <- ggdraw() + draw_label("Figure 5. Correlations between immune module expression in the upper respiratory tract", size=8, fontface='bold')
fig5_title2 <- ggdraw() + draw_label("and peripheral blood of SARS-CoV-2-infected children and adolescents", size=8, fontface='bold')

png(file="Figures/Figure_5.png", width = 7.5, height = 5.25, units = 'in', res = 1200)
plot_grid(fig5_title1, fig5_title2, NULL, fig5_plot, labels=NULL, ncol=1, rel_heights=c(0.03,0.03,0.02,0.97), align="v") 
dev.off()