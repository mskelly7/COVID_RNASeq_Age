...
fgsea_np_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/2_COVID_Pos_vs_Neg/modules_np_Pos_Neg.xlsx"))
...
fgsea_pax_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/2_COVID_Pos_vs_Neg/modules_pax_Pos_Neg.xlsx"))
...
fig2_plot <- plot_grid(title_np, plot_np, title_pax, plot_pax, labels=NULL, rel_heights=c(0.1,1,0.1,1), nrow=4, align="v") 
fig2_title <- ggdraw() + draw_label("Figure 2. Differential host gene expression in upper respiratory and peripheral blood samples associated with SARS-CoV-2 infection among children, adolescents, and adults", 
                                    size=13.5, fontface='bold')

png(file="Statistical_Analyses/Figures/Figure_2.png", width = 16, height = 10.5, units = 'in', res = 1200)
plot_grid(fig2_title, NULL, fig2_plot, labels=NULL, ncol=1, rel_heights=c(0.03,0.02,0.97), align="v") 
dev.off()