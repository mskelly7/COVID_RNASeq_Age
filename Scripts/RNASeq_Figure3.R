...
phy.rnaseq.np.Pos <- subset_samples(phy.rnaseq.np, corona=="Positive")
...
fsgea_np_0to5_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_np_0to5_Pos_Neg.xlsx"))
fsgea_np_6to13_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_np_6to13_Pos_Neg.xlsx"))
fsgea_np_14to20_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_np_14to20_Pos_Neg.xlsx"))
fsgea_pax_0to5_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_pax_0to5_Pos_Neg.xlsx"))
fsgea_pax_6to13_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_pax_6to13_Pos_Neg.xlsx"))
fsgea_pax_14to20_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_pax_14to20_Pos_Neg.xlsx"))
fsgea_pax_adult_Pos_Neg <- data.frame(read_excel("Statistical_Analyses/3_COVID_Pos_by_Age/modules_pax_adult_Pos_Neg.xlsx"))
...
modules_np <- rbindlist(list(fsgea_np_0to5_Pos_Neg, fsgea_np_6to13_Pos_Neg, fsgea_np_14to20_Pos_Neg))
...
innate_plot_np <- ggplot(modules_np_innate, aes(x = pathway, y = Sample)) +
  ...
  scale_size_continuous(range = c(14,20)) + ggtitle("Upper respiratory") +
  coord_flip() 
innate_plot_np
...
modules_pax <- rbindlist(list(fsgea_pax_0to5_Pos_Neg, fsgea_pax_6to13_Pos_Neg, fsgea_pax_14to20_Pos_Neg, fsgea_pax_adult_Pos_Neg))
...
innate_plot_pax <- ggplot(modules_pax_innate, aes(x = pathway, y = Sample)) +
  ...
  theme(legend.key=element_blank(), legend.position = "none", panel.background = element_rect(colour = "black", fill = NA, linewidth=0.8),
        ...
        plot.title=element_text(size=14, color="black", hjust=0.5, face="bold")) + 
  scale_size_continuous(range = c(14,20)) + ggtitle("Peripheral blood") +
  coord_flip() 
innate_plot_pax
...
module_plot_np <- plot_grid(innate_plot_np, NULL, adaptive_plot_np, labels=NULL, ncol=1, rel_heights=c(3.8,-0.06,2.25), align="v") 
module_plot_pax <- plot_grid(innate_plot_pax, NULL, adaptive_plot_pax, labels=NULL, ncol=1, rel_heights=c(3.8,-0.06,2.25), align="v") 
fig3_modules <- plot_grid(module_plot_np, NULL, module_plot_pax, labels=NULL, ncol=3, rel_widths=c(3.2,-0.05, 2.08), align="vh") 

# NO TITLE

#png(file="Statistical_Analyses/Figures/Figure_3.png", width = 16, height = 11, units = 'in', res = 1200)
#plot_grid(fig3_cibersort, fig3_modules, labels=c("a","b"), ncol=2, rel_widths=c(8,8), align="h") 
#dev.off()

# ADDITION OF TITLE

fig3_plot <- plot_grid(fig3_cibersort, fig3_modules, labels=c("","c"), ncol=2, rel_widths=c(8,8), align="h") 
fig3_title <- ggdraw() + draw_label("Figure 3. Transcriptional profiles associated with SARS-CoV-2 infection by age group", 
                                    size=15, fontface='bold')

png(file="Statistical_Analyses/Figures/Figure_3.png", width = 16, height = 11.25, units = 'in', res = 1200)
plot_grid(fig3_title, NULL, fig3_plot, labels=NULL, ncol=1, rel_heights=c(0.03,0.02,0.97), align="v") 
dev.off()